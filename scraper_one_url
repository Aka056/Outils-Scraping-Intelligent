...
# # pour la tokénisation les bibliothèques punkt permettent de segmenter un texte en mot
# nltk.download('punkt_tab')
# nltk.download('punkt') 
# nltk.download('stopwords')
# # Pour la lementisation, on utilise wordnet pour comprende le sens profond des mots afin de regrouper ceux qui signifie la mm chose 
# nltk.download('wordnet')

url="https://www.amazon.com/s?i=videogames-intl-ship&bbn=16225016011&rh=n%3A468642%2Cp_123%3A184411%257C218247%257C220854%257C221831%257C248671%257C358345%257C381900%257C395698&dc&qid=1747921882&rnid=85457740011&ref=sr_nr_p_123_8&ds=v1%3AjUsyaOpJbf2kGD1P%2FZJAX0AoksAUZDpNwVbC14qa6KY"
...
def clean_text(text): 
    text = text.lower()
    text = re.sub(r'[^a-zA-Z0-9\s]', '', text)  #supprime tt ce qui n'est pas compris entre A à Z, 0 à 9 et les espace "/s" c'est à dire alors(les emojies, les caractères spéciaux, les ponctuations et accents...)
    tokens = nltk.word_tokenize(text)
    tokens = [lemmatizer.lemmatize(word) for word in tokens if word not in stop_words and len(word) > 2]
    return ' '.join(tokens) # permet de regrouper les tokens pr en faire des phrases en remplaçant les virgule par des espaces ' '
...
try:
    ...
    if response.status_code == 200 :
        ...
        # f= open("DKLE.html", "w") # f pour file et "w" pour write on se met en mode écriture et on écris dans le fichier "products.html"
        # f.write(html) # bah on ecrit du coup le contenue de notre variable html
        # f.close() 
        ...
        df = pd.DataFrame(data)  # Liste avec un seul products

        df.to_csv("fake.csv", index=False, encoding="utf-8",  header=not pd.io.common.file_exists("fake.csv"), mode='a')
    ...
    async def main():
        ...
        async with async_playwright() as p:
            ...
            # with open("DKLE.html", "w", encoding="utf-8") as f:
            #     f.write(html)
...
print("Nombre de mots uniques dans le corpus :", len(set(' '.join(df['Cleaned_Product_Name']).split())))
